...
__rtld_malloc_init_real (struct link_map *main_map)
{
  /* We cannot use relocations and initializers for this because the
     ...
     relocations that do not depend on the previous pointer contents.
     Also avoid direct relocation depedencies so this function can be
     called before the final rtld relocation (which enables RELRO,
     after which the pointer variables cannot be written).  */
